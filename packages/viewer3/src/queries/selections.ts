...
export const SelectionsMutationKey = {
  setSessionSelections: (type: SelectionsType) =>
    ["sessionSelections", type] as const,
}

export const SelectionsQueryOptions = {
  byId: (service: SelectionsService | null | undefined, id: string) =>
    queryOptions({
  ...
  sessionSelections: (store: SelectionsStore, type: SelectionsType) =>
    queryOptions({
...
export const SelectionsMutationOption = {
  setSessionSelections: (store: SelectionsStore, type: SelectionsType) =>
    mutationOptions({
...
export const SelectionsLoader = {
  sessionSelections: async (
    contextPromise: Promise<AwaitedAppContextValue>,
    type: SelectionsType,
  ) => {
    const { queryClient, selectionsStore } = await contextPromise
    return queryClient.query({
      ...SelectionsQueryOptions.sessionSelections(selectionsStore, type),
      staleTime: "static",
    })
  },
}
